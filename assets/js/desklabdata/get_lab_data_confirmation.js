document.addEventListener('DOMContentLoaded', function () {
    ...
    const printBtn = document.getElementById('labPrintBtn');
    ...
    // ===== Fill Lab confirmation table dynamically =====
    window.fillLabConfirmation = function (dataObj) {
        let html = '';

        console.log("Lab Confirmation Data:", dataObj);

        // ===== Lab Header =====
        html += ` <div class="form-header">
                      <h2>LABORATORY SORTING DATA</h2>
                      <h3>Entomology Laboratory Sorting Data - ISCA - PHASE - III, 2025</h3>
                      <div class="meta">
        ...
        html += `<table class="info-table">
                    ...
                    <tr><td>Collection Date</td><td>${naValue(dataObj['field_coll_date'])}</td>
                        <td>Form Title</td><td>${naValue(dataObj['lab_frm_title'])}</td></tr>
                    <tr><td>Device ID</td><td>${naValue(dataObj['deviceid'])}</td>
                        <td>Recorded by</td><td>${naValue(dataObj['labrecname'])}</td></tr>
                 </table>`;

        // ===== Species - Anopheles gambiae =====
        html += `<table class="info-table">
                    <tr><th colspan="6">Anopheles gambiae</th></tr>
        ...
        html += `<table class="info-table">
                    <tr><th colspan="6">Anopheles funestus</th></tr>
        ...
        // ===== Other Species - OAN =====
        html += `<table class="info-table">
                    <tr><th colspan="6">Other Anopheles (OAN)</th></tr>
        ...
        // ===== Other Culicine =====
        html += `<table class="info-table">
                    <tr><th colspan="4">Other Culicine</th></tr>
                    <tr><td>Male</td><td>${naValue(dataObj['male_other_cul'])}</td>
                        <td>Female</td><td>${naValue(dataObj['female_other_cul'])}</td></tr>
                 </table>`;
    ...
    submitBtn?.addEventListener('click', async () => {
        ...
        try {
            const res = await fetch(BASE_URL + '/api/datacheck/uploadlabapi/add_lab_sorting.php', {
                method: 'POST',
            ...
            if (resp.success) {
                showToast("Lab data submitted successfully!", "success");
                wizardForm.reset();
            } else {
                showToast(resp.message || "Error submitting lab data.", "error");
            }
        } catch (err) {
            console.error(err);
            showToast("Server error while submitting lab data.", "error");
        }
    });
    ...
    printBtn?.addEventListener('click', () => {
        ...
        const w = window.open('');
        w.document.write('<html><head><title>Lab Confirmation</title></head><body>');
    ...
    exportPdfBtn?.addEventListener('click', async (e) => {
        ...
        // ==== Footer ====
        const fullName = document.querySelector('input[name="lab_tech_name"]')?.value || 'Unknown';
        const dateStr = new Date().toLocaleString();
        pdf.setFontSize(10);
        pdf.text(`Prepared by: ${fullName}`, 40, pdf.internal.pageSize.getHeight() - 40);
