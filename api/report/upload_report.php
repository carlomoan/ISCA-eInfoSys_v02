...
// Insert into DB
try {
    $stmt = $pdo->prepare("INSERT INTO uploaded_reports (file_name, report_type, round, cluster_name, uploaded_at, file_path) VALUES (?, 'Uploaded', ?, ?, NOW(), ?)");
    $stmt->execute([$filename, $round, $cluster_name, $newFileName]);
} catch (Exception $e) {
    error_log("Upload DB insert error: " . $e->getMessage());
    http_response_code(500);
    echo json_encode(['success' => false, 'message' => 'Failed to save report record.']);
    exit;
}
