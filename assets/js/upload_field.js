document.addEventListener('DOMContentLoaded', () => {
  const form = document.getElementById('upload-field-form');
  const feedback = document.getElementById('field-upload-feedback');
  const summary = document.getElementById('upload-summary');
  const preview = document.getElementById('field-upload-preview');
  const missingContainer = document.getElementById('missing-households-container');
  ...
  function renderPreviewTable(data) {
    ...
    let html = '<h3>📄 Preview of Collected Field Data</h3>';
      ...
      <table style="border-collapse: collapse; width: 100%;">
        <thead>
          <tr>
            ...
            <th style="border:1px solid #ccc; padding:6px;">Field Recorder</th>
    ...
    if (hasMissing && missingHTML.trim() !== '') {
      ...
      missingContainer.innerHTML = `
        <div style="margin-top:20px; text-align:right;">
          <button id="next-to-lab-btn" style="
      ...
      const nextBtn = document.getElementById('next-to-lab-btn');
      nextBtn.addEventListener('click', () => {
        document.querySelectorAll('.tab-btn').forEach(btn => btn.classList.remove('active'));
        const labTabBtn = document.querySelector('.tab-btn[data-tab="lab"]');
        if (labTabBtn) labTabBtn.classList.add('active');

        document.querySelectorAll('.tab-content').forEach(tab => tab.classList.remove('active'));
        const labTabContent = document.querySelector('.tab-content[data-tab="lab"]');
        if (labTabContent) labTabContent.classList.add('active');
      });
    }
